...
## Slide 6: How Wasm Improves eBPF Developer Experience

Stepping into the developer's shoes, we see how Wasm refines the eBPF development workflow.

## Slide 7-10: eBPF Deployment Models (4 minutes)
...
## Slide 13-14: WasmEdge eBPF plugin (1 minutes)
...
We can also list the existing podman WebAssembly containers, and see the container we just started, or stop and remove the container.
...
## Slide 20: The Operational Framework of wasm-bpf (1 minutes)
...
Communication is a two-way street with wasm-bpf. It sets up a path for back-and-forth conversations with the kernel using eBPF Maps, making data transfer smooth and efficient with ring buffers. The eBPF can efficient sending of messages from the kernel state to the user state via `ring buffering` and perf event polling, or accessing hashmaps from the Wasm virtual machine. The bpf maps can also be accessed with share memory between kernel and Wasm runtime. This setup is not only flexible but also ready to grow with new kernel features without the need to modify the virtual machine's system interface.

## Slide 21: The wasm-bpf Development Process (1 minutes)
