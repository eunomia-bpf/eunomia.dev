...
description: AI 编程 Agent 在平台方可能并不拥有的 Harness 与沙箱中自主运行数小时，基于审批的管控随之失效。本文主张将 Agent 安全拆分为三层（意图授权、执行隔离、副作用验证），并用基于 eBPF 的可观测（AgentSight）与执行控制（ActPlane）在 Harness 之下提供独立的证据与策略平面。
---
...
harness 执行控制，两者都使用 eBPF 在 Agent harness 之下提供独立的证据和
策略平面。
...
2026 年的一项行业调查发现，[65% 的企业报告了 AI Agent 安全事件](https://www.kiteworks.com/cybersecurity-risk-management/ai-agent-security-incidents-2026/)，
其中大多数涉及缺乏适当 Agent 访问控制的组织。
...
与模型本身同样关键。GitHub 的 Agentic 工作流架构从这一前提出发：
...
解耦](https://www.anthropic.com/engineering/managed-agents)，将容器视为
可丢弃的牲畜（cattle），确保 token 永远不会从运行生成代码的沙箱中可达。
...
可以在 Agent 外面包裹自己的沙箱和证据层。当 Agent 运行在提供商管理的环境中
时，独立的证据平面必须转移到平台团队确实控制的边界上。

这就产生了问责缺口：**平台团队要为一个无法完全审查的工作负载的生产影响负责，
而该工作负载运行在一个可能不属于自己的沙箱中。**

旧的心智模型很简单：Agent 有风险，把它放进沙箱。新的现实有着不同的信任边界：
Agent 及其 harness 是工作负载的一部分，环境拥有者需要一个独立的证据平面。

## 三层架构，三个问题

MCP、沙箱和操作系统级证据对 Agent 安全都是必要的。它们不可互换。每一层
...
**副作用验证**（OS/运行时证据）回答：*实际*发生了什么？运行了哪些进程，
...
如前所述，用户在 93% 的提示上直接同意，产品主动提供绕过模式。当审批放松时，
其他两层必须进行补偿。如果你自动批准常规操作，你需要独立的方式来验证这些
操作实际做了什么。如果你为了速度绕过权限，你需要更强的隔离和更强的证据。

### Harness 不透明性

当 harness 不透明时，应用层遥测不能作为唯一的证据来源。OpenTelemetry GenAI
...
在所有三种情况下，OS/运行时证据层都是让平台团队发现问题的独立控制手段，
无论哪一层失效。
...
这些证据在应用层之下采集，通常通过 eBPF、审计子系统或内核插桩。它不需要
修改 Agent 应用。其关键特性是独立性：证据由环境运营者拥有和采集，而非
Agent 提供商。
...
OS 证据：         agent → shell → python → curl
                  read: /var/run/secrets/.../token
                  connect: 未知外部主机
```

每一层看到了事件的不同部分。OS 证据层使平台团队能够发现不匹配。

## 部署现实

OS 级证据在你控制 Agent 执行所在的主机、节点或 VM 时最为强大。如果 Agent
完全运行在提供商管理的环境中，你可能无法在其中挂载 eBPF。

在这种情况下，同样的模型适用，但证据转移到你确实控制的边界上：
...
这些证据比拥有运行时边界要弱，但仍然好过将 Agent 的对话记录作为唯一的
事实来源。
...
不透明、闭源，或运行任意绕过框架级追踪的子进程，它也能正常工作。
...
两个工具是互补的。AgentSight 提供证据平面：独立的、应用层之下的关于 Agent
行为的事实。ActPlane 提供执行控制平面：确定性的、内核级的关于 Agent 不能
...
拥有的唯一一层就是 OS/运行时证据平面。

MCP 授权意图。沙箱约束执行。OS 证据验证副作用。每一个都是必要的；没有一个
是充分的。实践模型是它们的分离：
...
- [AgentSight blog post](agentsight_paper.zh.md)
