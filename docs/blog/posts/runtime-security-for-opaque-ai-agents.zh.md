...
保证"。该框架留下的问题是：当某一层发生失败时，部署方是否有独立的证据来发现
它。在云基础设施中，共担责任模型中类似的缺口（客户"拥有"配置但无法观测运行时
行为）催生了 CloudTrail、Config、GuardDuty 等由客户控制的独立证据服务。Agent
...
每一层看到了事件的不同部分。没有 OS 证据层，这就是一次未被检测到的凭证窃取：
服务账号 token 被读取并发往外部主机，而框架日志只记录了"运行测试"。平台团队
数天后才发现泄露，如果能发现的话。OS 证据层将一次隐形的数据泄漏变成实时检测。
...
这也回应了 Anthropic 在其可信 Agent 框架中指出的生态缺口：对跨部署证据共享
和 Agent 安全开放标准的需求。独立的运行时证据随工作负载迁移，而不是被锁定在
