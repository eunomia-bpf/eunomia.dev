...
本文对应我们的论文 [**ActPlane: Programmable OS-Level Policy Enforcement for Agent Harnesses**](https://arxiv.org/abs/2606.25189)。当前 arXiv 版本包含策略实证研究、coding task 与安全 benchmark 评测，也量化了把执行控制下沉到工具层以下的成本。实现和复现实验材料位于 [eunomia-bpf/ActPlane](https://github.com/eunomia-bpf/ActPlane) 仓库。
...
论文使用实证研究中收集的策略，并结合 coding task 与安全 benchmark 做评测。实验覆盖工具调用拦截无法观察的间接执行路径，例如 Agent 生成脚本后再通过多层子进程触发命令。在被评测的配置中，ActPlane 提高了策略遵从率，开销为 1.9% 到 8.4%。这些结果量化了内核级覆盖的成本，也检验了触发执行控制后，带有原因的反馈能否帮助 Agent 改换路径继续完成任务。
