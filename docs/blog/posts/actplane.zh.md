...
MCP gateway、AgentSpec、工具级权限控制在约束表面上更靠近实际操作。如果 Agent 调用 `file_write("/etc/passwd", ...)`，工具层可以在 API 入口拒绝。这比 prompt 约束可靠得多，因为它检查的是操作本身而不是意图。
...
容器、VM、E2B、Daytona 用隔离边界包裹整个执行环境。这是目前最可靠的安全边界：沙箱内的进程无法访问沙箱外的资源。对于"防止 Agent 逃逸到宿主机"这个问题，沙箱是正确的答案。
...
三层约束的盲区指向同一个方向：约束机制需要下沉到所有操作的必经之路。Agent 用什么工具、写什么脚本、spawn 多少层子进程，最终所有副作用都要经过操作系统内核。每一次 exec、每一次文件 open、每一次网络 connect、每一次 fork，内核都在场。ActPlane 在这个层面工作：通过 eBPF 在内核中安装轻量级程序，hook 住 Agent 可能产生副作用的所有路径，然后根据标签化的信息流策略做决策。
...
ActPlane 的策略不是静态的允许/拒绝列表。它用的是标签化的信息流策略（labeled information-flow policies）：给进程和文件打标签，标签沿 fork/exec 边和文件读写边自动传播，规则基于标签做决策。听起来抽象，但跟着一个例子走一遍就很清楚。
...
用户态部分是 `actplane` 这个 Rust 二进制。它发现并解析 `actplane.yaml`，将 DSL 编译为内核配置（写入 eBPF 程序的 `.rodata` 段），通过 aya 这个 Rust eBPF 库在进程内加载预编译的 eBPF 对象（不依赖 libbpf 或 clang），设定目标进程的谱系种子（告诉内核"从这个进程开始追踪"），然后监听 ring buffer 报告规则匹配及其策略原因。
...
ActPlane 的内核级标签传播机制并非全新。[CamQuery](https://dl.acm.org/doi/10.1145/3243734.3243776)（Pasquier 等，CCS 2018）已经在内核中基于 [CamFlow](https://dl.acm.org/doi/10.1145/3127479.3129249) 溯源图实现了跨通道的 taint 传播和执行。ActPlane 的贡献在于将这个机制搬到了现代 eBPF/BPF-LSM 基础设施上（无需内核模块），面向的是 cooperative-but-forgetful 的 AI Agent 威胁模型而非远程攻击者，并且加入了面向 Agent 的纠正性语义反馈。

在 eBPF 执行层面，[Cilium Tetragon](https://tetragon.io/) 提供了 `matchBinaries` + `followChildren` 沿 fork/exec 传播二进制谱系标记，是与 ActPlane 谱系追踪最接近的开源功能，但只沿进程边传播，不跨文件/网络边，也没有语义反馈。[OAMAC](https://arxiv.org/abs/2601.14021)（2026）通过 BPF-LSM 传播执行来源标签，但同样只限于进程创建，不覆盖文件 I/O。两者都不是面向 Agent 的。

在 Agent guardrail 层面，[AgentSpec](https://arxiv.org/abs/2503.18666)（Wang 等，ICSE 2026）是最接近 ActPlane 纠正性反馈理念的工作，提供了"corrective invocation"和"self-reflection"机制。[Progent](https://arxiv.org/abs/2504.11703)（2026）通过符号规则做确定性的工具调用权限控制。两者都在 Agent 框架内的工具调用 API 层执行，可以被 shell out 或直接 SDK 调用绕过。[SAFEFLOW](https://arxiv.org/abs/2506.07564)（2025）在 Agent 协议层引入了带溯源追踪的信息流控制，但在编排层而非 OS 内核层工作。ActPlane 在工具层之下执行，补全了这些方案的覆盖盲区。

观测基础来自 [AgentSight](https://arxiv.org/abs/2508.02736)（2025），它用 eBPF 同时捕获 Agent 的意图层和行为层数据，提出了 Agent 意图和系统级实际行为之间的"semantic gap"概念。ActPlane 将这个观测流变成了一个带执行力的策略引擎。
