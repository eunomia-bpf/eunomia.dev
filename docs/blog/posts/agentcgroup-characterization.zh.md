---
date: 2026-02-17
description: AgentCgroup 刻画 AI 编程 Agent 的突发资源行为，并通过 eBPF、sched_ext 和 cgroup v2 在工具调用粒度控制 CPU 与内存，同时保留 Agent 已积累的执行状态。
---

# AgentCgroup：当 AI 编程 Agent 遇到操作系统资源

一个 AI 编程 Agent 安静地读了几分钟代码，改完文件后启动 `pytest`。测试进程加载依赖时，内存可能在一秒内增加数百 MB，命令退出后又迅速回落。容器级 controller 只看到整个 workload 越过阈值，却分不清这次 burst 来自短命的工具进程，而长时间运行的 Agent runtime 还保存着对话、诊断过程和未完成的修改。

为了知道这种情况多常见，我们使用两个 LLM 后端运行了 144 个 SWE-rebench 任务。容器初始化和工具执行等 OS 工作占端到端延迟的 56% 到 74%，内存峰值达到平均值的 15.4 倍，亚秒级变化速度最高为 3 GB/s。即使同样标记为 Bash 的两次调用，内存需求也可能相差 13.7 倍，因为一条命令只是运行 `git status`，另一条却启动整套测试。Token 数量几乎无法预告下一次峰值，它与峰值内存的相关性在 Haiku 上为 -0.14，在 GLM 上为 +0.02。

静态分配很难处理这种组合。按观察到的峰值预留资源，会在安静阶段浪费最多 93% 的已配置容量；在峰值时杀掉容器，又会丢掉 Agent 已经积累数分钟且无法确定性重放的状态。[AgentCgroup 论文](https://arxiv.org/abs/2602.09345)从这些测量出发，设计了一个能够在工具调用粒度响应的 eBPF resource controller。
...
为了把 `pytest` burst 对应到真正触发它的进程，我们对 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 进行了插桩，让它运行来自 [SWE-rebench](https://github.com/swe-bench/SWE-ReB) 的 144 个软件工程任务。Trace 同时保存工具调用边界与一秒粒度的 CPU、内存采样，因此容器内存的每次上升都能对应到当时执行的命令。实验覆盖两个 LLM 后端：
...
这些任务覆盖 CLI tools、build systems、科学与医疗代码、数据处理和 web projects，并包含三个难度等级。刻画阶段不施加资源限制，因为 controller 做出处理决定之前，需要先看到未受约束的真实峰值。

## Tool Call 才是 Spike 的来源

开头那个 `pytest` process 并非隐藏在模型推理下面的偶发现象。两个 backend 的端到端时间大多花在启动环境和运行工具上，操作系统因此直接位于关键路径。
...
每个任务随后会存活 5 到 11 分钟，把 LLM context、代码修改和工具结果保存在一个有状态进程中。Bash 与 sub-agent calls 占工具时间的 90% 以上，不过 Haiku 会把更多工作交给 sub-agents，GLM 则几乎全部通过本地 Bash 完成。Controller 因而需要保护长时间运行的 runtime，同时区分同一个 Bash 工具下面需求完全不同的命令。

### 沿着 Bash 再往下一层

工具名称仍然无法说明资源需求。把 Bash calls 按命令语义拆开后，时间分布如下：
...
## Burst 结束得比容器 Controller 的调整更快

在我们的 24 核平台上，即使并发数量已经达到内存上限，CPU 仍保持在 36% 以下。每个任务的峰值内存达到 2 到 4 GB，按峰值给每个容器分配资源会先耗尽整台机器的内存，同时留下大量空闲 CPU。Trace 给出了原因：一层稳定内存需要保护，另一层只在工具执行时短暂出现。

### 保护 185 MB 的有状态 Baseline

Agent memory 由两层组成。开头的 `pytest` 属于短暂出现的一层，对话与 framework 则位于下面的稳定层。
...
把每个 sample 对应到当时的 tool call 后，内存 burst 相对工具执行时间的集中程度达到 1.9 倍。CPU burst 更分散，尤其是本地 inference 还会带来后台 CPU load，因此 CPU 和内存需要各自的控制信号。
...
如果按照 4060 MB 峰值给 pydicom 容器分配内存，它在通常只有 264 MB 的阶段会浪费 93% 的容量。把 limit 设在平均值附近，又会让一秒钟的工具 burst 触发 OOM kill，并丢掉需要保护的 baseline。策略需要辨认制造临时内存层的具体命令，因此我们继续比较 Bash 内部的 memory spikes：
...
Medical 与 bioinformatics 命令的平均峰值达到 4 GB，web 和 network 命令则为 291 MB。Bash-level budget 仍会把 `ls`、`git status` 与 `pytest` 放在一起，真正有用的边界位于实际命令进程。CPU 也无法代替内存作为信号，因为不同任务中两者的相关性从 -0.84 到 +0.50 不等。

## 下一次 Pytest 不会复现上一次的轨迹

如果昨天的 trace 能够预测明天的峰值，静态 limit 仍可能奏效。把完全相同的 `iterative/dvc#777` 任务运行三次，可以看到 Agent 为什么无法依赖这段历史：
...
模型自身的活动同样很难提前预警。LLM 可观测 proxy 与实际资源消耗的相关性依然很弱：
...
全部任务的峰值内存从 197 MB 到 4 GB，相差 20 倍，即使使用同一个 Agent framework 也是如此。更换模型还会再次改变 profile。Controller 因而需要当前命令边界与实时压力，历史运行的 percentile 无法替代这两个信号。

## 给 Tool Call 单独的 Resource Domain

回到开头的 `pytest` burst。185 MB Agent runtime 与它的测试 child 目前共享同一个容器 limit，虽然 runtime 保存着昂贵状态，临时 allocation 却来自 child。AgentCgroup 把 Agent workload 映射为 cgroup v2 node，再把每次工具调用放入 child node。`git status` process 与 `pytest` process 由此可以使用不同约束，同时继续受 workload 总预算控制。

这个层次也改变了恢复方式。超过 soft limit 时，系统可以冻结或 throttle 工具子树，让 parent Agent 继续存活。确实需要终止时，cgroup v2 可以原子化 kill 这个子树，保留 runtime 中的对话和修改。

## 把响应移进内核

命令边界解决了粒度问题，一秒钟内以 3 GB/s 变化的 burst 仍要求快速响应。AgentCgroup 通过 eBPF 在 kernel cgroup enforcement points 执行控制逻辑，省去用户态 signal、decision 和 write-back loop：
...
同一组内核观测使用当前证据代替历史预测。AgentCgroup 跟踪进程创建与内存分配，识别 tool boundaries，并随着压力变化逐步响应：`memory.high` delay 控制 allocation，`cgroup.freeze` 暂停子树，`memory.max` 保留为 hard boundary。Parent runtime 继续保存工具返回或失败后做下一步决策所需的状态。
...
测得的 P50 latency overhead 为 0.3%，BPF throttling precision 的相对误差在 2.3% 以内。这些数字来自 proof-of-concept kernel path 上的 trace replay，因此它们展示的是论文实验环境中的控制机制，还不代表生产规模下的行为。

## 如何复现实验结果

最快的检查路径先使用仓库中已经采集好的 traces，在用户态重新生成 figures，随后再进入需要加载 eBPF 程序的实验：
...
[复现指南](https://github.com/eunomia-bpf/agentcgroup/blob/main/docs/REPRODUCING.md)继续给出 CPU scheduling、memory isolation 和 overhead 实验对应的命令。CPU 控制需要 Linux 6.12 或更新版本，并启用 `sched_ext` 与 cgroup v2。内存实验还使用 artifact 描述的 `memcg_bpf_ops` 内核路径，因此这部分需要匹配的 kernel support 和 root 权限。

## 证据的边界在哪里

当前刻画覆盖 Claude Code 与 SWE-rebench，controller evaluation 则使用 proof-of-concept implementation 加速重放 traces。真实并发 Agent、OpenHands 与 SWE-agent 等其他 frameworks、更多 container runtimes，以及 `memcg_bpf_ops` 的 upstream 状态仍待验证。[论文](https://arxiv.org/abs/2602.09345)与 [eunomia-bpf/agentcgroup](https://github.com/eunomia-bpf/agentcgroup) 仓库公开了原始实验、分析脚本、controller code 和复现说明，可以据此继续检验这些边界。
