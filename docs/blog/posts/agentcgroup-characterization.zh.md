...
为了知道这种情况多常见，我们使用两个 LLM 后端运行了 144 个 SWE-rebench 任务。容器初始化和工具执行等 OS 工作占端到端延迟的 56% 到 74%，内存峰值达到平均值的 15.4 倍，亚秒级变化速度最高为 3 GB/s。即使同样标记为 Bash 的两次调用，内存需求也可能相差 13.7 倍，因为一条命令只是运行 `git status`，另一条却启动整套测试。Token 数量几乎无法预告下一次峰值，两者相关性只有 r = -0.14。
...
检查论文中的刻画证据不需要先加载 eBPF 程序。仓库已经包含采集好的 traces，可以直接在用户态重新生成 figures：
...
[论文](https://arxiv.org/abs/2602.09345)、代码、原始实验数据、分析脚本和复现说明均位于 [eunomia-bpf/agentcgroup](https://github.com/eunomia-bpf/agentcgroup) 仓库。
