...
真正麻烦的地方在于，这些特征刚好踩在现有资源管理机制的盲区上。从内核 cgroup limits、systemd-oomd 到 Kubernetes VPA，静态资源分配要么浪费 93% 的已配置容量，要么触发 OOM kill，摧毁 Agent 已经积累数分钟、且无法确定性复现的执行状态。本文总结我们的 [AgentCgroup 论文](https://arxiv.org/abs/2602.09345)中的刻画结果，并说明基于 eBPF 的内核态执行控制如何弥合 Agent 工作负载动态与 OS 资源控制之间的 gap。
...
公开的 [AgentCgroup artifact](https://github.com/eunomia-bpf/agentcgroup) 把 controller、实验和分析材料放在同一个仓库中。`agentcg/` 目录包含 daemon、process monitor、`sched_ext` scheduler 和 memory controller；`scripts/`、`experiments/`、`analysis/` 则分别保存论文使用的 trace collection pipeline、原始测量数据和绘图分析代码。

仓库中的[复现指南](https://github.com/eunomia-bpf/agentcgroup/blob/main/docs/REPRODUCING.md)把命令对应到论文的三组证据：第 4 节的工作负载刻画、第 5 节的多租户资源控制实验，以及第 6 节的 controller overhead 测量。CPU 实验需要支持 `sched_ext` 的 Linux kernel 和 cgroup v2。内存控制实验还使用 artifact 中描述的 `memcg_bpf_ops` 路径，因此复现这部分结果需要相应的内核支持，普通旧版发行版内核无法直接完成。
