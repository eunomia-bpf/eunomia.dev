...
这些层留下的是同一道裂缝。大多数规则需要存在于 agent 处的项目或任务上下文，因此 agent 自身必须能将策略转化为具体规则；然而许多策略定义事件顺序或数据流，工具调用层面的守卫根本看不见，所以规则仍必须足够具体，才能落到确定性的 OS 级强制执行。弥合这个差距正是 ActPlane 要解决的问题。

由此推出两项设计要求。策略规范必须既可被 agent 编写，又可在 OS 执行，这样 agent 才能用最少的专业知识把自然语言策略落成具体规则，并靠语义反馈理解违规、完成恢复。强制执行还必须安全、隔离且高效，也就是 agent 编写的策略不能削弱更高权限设置的约束，不能影响其他 agent 的策略，也不能拖慢 agent 的正常工作负载。
