...
description: 对 CLAUDE.md 和 AGENTS.md 中 2116 条规则语句的实证分析表明，64% 是需要强制执行的策略，其中 83% 涉及系统可观测行为，74% 依赖项目或任务上下文。基于 eBPF 的内核级强制执行加语义反馈将决策合规率提升至 75.8%。
---

# AI Agent 的安全规则需要上下文，也应该由系统强制执行：2116 条规则实证

一个 AI coding agent 执行 `git commit`，内核没有看到任何异常：一个熟悉的进程在写熟悉的文件。仓库的 CLAUDE.md 里写着"提交前必须跑完整测试套件"，而这个 agent 在上次测试之后又修改了源码。这条规则来自论文数据集中的真实语句（statement），今天没有任何一层在强制执行（enforcement）它。

[ActPlane 论文](https://arxiv.org/abs/2606.25189)量化了这道裂缝。论文度量了开发者在 CLAUDE.md 和 AGENTS.md 里实际写下的策略（policy），逐条分类了强制执行这些策略需要什么条件，并验证了 OS 级强制执行加语义反馈（semantic feedback）能否真正生效。论文的全景概括："64% 的语句是策略，83% 涉及系统行为，74% 依赖无法静态预定义的上下文（context）。"（原文："64% of statements are policies, 83% involve system actions, and 74% depend on context that cannot be pre-defined statically."）
...
1361 条策略中只有 17% 是纯语义的，其余 83% 是系统可观测的，即内核级监测器原则上可以评估：38% 需要内容检测，29% 匹配单个 OS 事件，16% 需要跨事件状态。跨事件策略集中在开发流程领域，占全部跨事件策略的 39.5%。
...
微基准测试定位了单次系统调用开销的分布。fork 和 exec 的绝对增加量最大，跨规则数量范围为 3 到 69 微秒，但相对于 49 和 248 微秒的原生延迟仍然适度。文件和网络操作保持轻量：open 在 100 条规则时达到 13.4 微秒，write 和 connect 保持亚微秒级。整个工具调用的系统调用序列累积的 ActPlane 开销比单次 2 到 10 秒的 LLM 推理轮次小五到六个数量级。策略更新传播迅速：通过用户空间 ring buffer 提交的单规则热重载平均 26.3 微秒到达内核排空路径，立即的 exec 违规在 p50 为 176.4 微秒内被检测到（包含进程启动和事件传递）。
...
eBPF 对 OS 事件（文件写入、进程启动、网络连接等）提供确定性强制执行，覆盖了 83% 的系统可观测策略。但剩余 17% 的纯语义策略（涵盖推理、沟通风格或输出质量）在内核之外，需要 harness 层控制来处理。内容检测策略（需要读取文件内容的 38%）更适合由代码检查器和静态分析器处理，不在 eBPF 的单事件和跨事件强制执行范围内。任务意图、策略权限、内容语义和隔离都是内核强制执行点周围的层需要承担的职责。
