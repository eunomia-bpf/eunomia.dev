...
一个 AI 编程 agent 执行 `git commit` 时，内核只看见熟悉的进程在写熟悉的文件。仓库的 CLAUDE.md 里写着“提交前必须跑完整测试套件”，而这个 agent 在上次测试之后又修改了源码。常见的 prompt 与沙箱组合通常没有共享状态，因此无法记录上次测试结果已经因后续源码修改而失效。
