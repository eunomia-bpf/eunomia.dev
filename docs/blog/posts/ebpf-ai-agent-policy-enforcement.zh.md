...
跨事件策略反复出现为四类关系。时序排序（temporal ordering）约束先后关系，像“提交前先跑测试”这样的要求关心的是一件事发生在另一件之后，而不是更早的某个时刻跑过就算数。跨文件一致性（cross-file consistency）把“行为变更时同步更新文档”中的源码编辑和文档更新联系起来。多步工作流（multi-step workflow）则要求发布清单逐项通过验证，上一步完成后才能进入下一步。条件触发（conditional trigger）只在前置条件成立时生效，例如“改了规范就必须同步 SDK”。单个事件无法判断这些关系，强制执行必须记录运行过什么、发生顺序以及此后的变化。这类策略并不少见，81% 的仓库至少包含一条，43% 的仓库横跨全部四个强制执行层级。
