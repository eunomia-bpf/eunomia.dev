...
> 更新（2026 年 3 月）：本文是历史版本发布说明，其中部分托管示例 URL 已不再维护。当前推荐的快速体验方式是从 GitHub Releases 安装 `ecli`，并运行 `sudo ./ecli run ghcr.io/eunomia-bpf/execve:latest`。
...
1. [当 Wasm 遇见 eBPF ：使用 WebAssembly 编写、分发、加载运行 eBPF 程序](https://eunomia.dev/blogs/ebpf-wasm/)
2. [如何在 Linux 显微镜（LMP）项目中开启 eBPF 之旅？](https://eunomia.dev/blogs/lmp-eunomia/)
