...
1. Task A 在 CPU 0 进入 classic uprobe 程序，开始使用该程序的 CPU-0 私有栈。
2. Uprobe runner 调用 `migrate_disable()`，因此 Task A 不会迁移到其他 CPU，但这本身不会禁止抢占。
3. Task A 在寄存器或 verifier 跟踪的指针已经 spill 到私有栈时被抢占。
4. Task B 在 CPU 0 运行、命中同一个 uprobe，并调用同一个 BPF 程序。
5. Task B 复用并覆盖同一份 per-program、per-CPU 私有栈 slot。
6. Task A 恢复后 reload 已损坏的 spill。此时 native code 使用的值不再符合 verifier 证明过的 pointer type。
...
最后一条命令要求调用者能够读取 JIT 指令。保存匹配的 BPF object、BTF、程序 ID/名称、attach type、translated instructions、JIT dump 与精确 kernel build。原始 panic 输出应留在 incident system；除非已经检查其中没有 secret 和私有拓扑，否则不要放进公开 issue 或内容仓库。
...
- draft application patch 用 weak `bpf_preempt_disable()` 和 `bpf_preempt_enable()` kfunc 包住所有可能从 uprobe 进入的程序主体。Tail call 需要特殊处理，因为 verifier 禁止在仍关闭抢占时 return 或 tail-call。只有所有入口、出口与 tail-call path 都覆盖时，这种方法才会阻止第一次调用被抢占。
...
- [Linux commit：选择可使用私有栈的 BPF subprogram](https://github.com/torvalds/linux/commit/a76ab5731e32d50ff5b1ae97e9dc4b23f41c23f5)
- [Linux commit：为 x86 JIT 增加 per-subprogram、per-CPU 私有栈](https://github.com/torvalds/linux/commit/7d1cd70d4b16ff0216a5f6c2ae7d0fa9fa978c07)
- [Linux commit：实现 sleepable uprobe 使用的 program-array path](https://github.com/torvalds/linux/commit/8c7dcb84e3b744b2b70baa7a44a9b1881c33a9c9)
- [公开 kernel-panic 报告与 reproducer 引用](https://github.com/open-telemetry/opentelemetry-ebpf-instrumentation/issues/3056)
- [使用 BPF preemption kfunc 的 draft application guard](https://github.com/open-telemetry/opentelemetry-ebpf-instrumentation/pull/3059)
- [实验性 kernel patch：用 per-program recursion context 保护 classic uprobes](https://github.com/simonswine/beyla/blob/20c9912a02c5d5924c992faf7ee353c7d71c9fa0/evidence/kernel-6.18-bpf-3733/0001-bpf-guard-classic-uprobes-against-private-stack-corr.patch)
- [bpftool 程序检查与 JIT dump 文档](https://github.com/libbpf/bpftool/blob/main/docs/bpftool-prog.rst)
- [BPF 邮件列表提案：folio-backed scratch 与 pool allocator](https://lore.kernel.org/bpf/20260817-folio-pool-v1-v1-0-0c1d230aa3af%40gmail.com/T/#t)
- [BPF 邮件列表提案：最大 16 字节 aggregate return value](https://lore.kernel.org/bpf/20260817042141.2286086-1-yonghong.song%40linux.dev/T/#t)
- [Prempti 架构：由 Falco rules 评估 tool-call hook](https://prempti.falco.org/)
- [ActPlane 架构：面向 agent process tree 的 kernel-level policy enforcement](https://github.com/eunomia-bpf/ActPlane)
- [Crash-safe eBPF dataplane loader 设计与实现](https://erwinkok.org/posts/ebpf-dataplane-loader/)
