...
问题可能来自公开 issue、邮件列表、论坛或技术社区。发布前，我们会重新表述问题，删除身份、私有部署细节和可能定位原讨论的措辞，再用公开资料核对答案。这里每天发布一篇，中英文各一篇；如果当日窗口有无法覆盖的部分，会在页面中如实说明这一缺口，而不是中止发布，也绝不用虚构内容填补。当日问题优先取自最近 24 小时；窗口稀疏时回退到最近 7 天；仍不足时，则从允许的可读历史归档、真实的未解决社区问题、允许的公开社区页面和公开一手资料中挑选可核实的真实问题，直到完成发布。Slack、Discord、Reddit 的覆盖不完整、不可访问或稀疏都只作为缺口记录，不是发布门槛；无法访问的频道记录为不可访问，绝不写成安静。
