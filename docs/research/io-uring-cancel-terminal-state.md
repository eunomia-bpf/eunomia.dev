...
For network I/O, the peer can record bytes observed. For file writes, the harness can read back the target after the terminal event and after forced shutdown. For poll/multishot operations, it can count events emitted before and after the final CQE. For zero-copy send, it can attempt buffer mutation between the result CQE and notification CQE to verify that the harness catches premature reuse. For io-wq paths, controlled blocking points can widen the `-EALREADY` race window.
