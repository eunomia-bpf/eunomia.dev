...
The `2026-08-24` report advances a different security boundary: how eBPF
information-flow enforcement can retain useful precision when one long-running
process multiplexes public and sensitive work while userspace TLS, kTLS, proxies,
and hardware offload move the observable transport boundary. It develops
sub-process flow generations, a trusted semantic-to-kernel binding, explicit TLS
path coverage, and a counterexample benchmark that measures false allows, false
denies, and unknown coverage.
...
2. `2026-08-24`: `/research/ebpf-cross-boundary-information-flow/` separates
   conservative whole-process/object taint from flow-specific application data
   provenance. It asks how semantic labels can be bound to enforceable kernel
   operations across shared runtimes and changing TLS paths without trusting
   stale fd, pointer, buffer, or request identities. The report is eBPF-centered
   because BPF LSM/socket enforcement and coverage are central to the mechanism.

### Preferred next questions

1. verifier and runtime interfaces for richer stateful policies;
2. portable policy execution between kernel, userspace, NIC, and DPU targets;
3. zero-copy and programmable I/O paths across XDP, AF_XDP, io_uring, DPDK, and
   userspace eBPF only when fresh evidence identifies a security/networking
   question that does not repeat the earlier io_uring control-surface,
   heterogeneous-placement, or application-resource reports;
