...
1. Preserve the rolling ten-report mix mechanically from the actually published archive. Before the `2026-09-09` publication the newest ten contain **5 eBPF-centered / 0 pure Agent / 5 adjacent systems**. The native-operation trust report is genuinely eBPF-centered because the BPF verifier boundary, JIT/kfunc/native execution, proof-linked fallback, implementation effects, and trusted-code identity are the central objects. It rotates the `2026-08-28` eBPF-centered proxy-identity report out, so the newest-ten mix remains **5 / 0 / 5** after publication. Never repair the ratio through classification or by publishing an extra report.
2. **eBPF Optimization and Execution Specialization** is the active series. The September 5 report established verifier safety versus optimizer equivalence and profile-assumption lifetime. September 6 established architecture-specific implementation eligibility, portable semantic witnesses, and deterministic fallback. September 7 established postmortem provenance for the exact specialization generation that executed. September 9 advances a distinct fourth boundary: how much native implementation code becomes trusted after delegation, which effects it may have, what assurance supports it, and how that evidence is bound to the artifact that executes.
3. Keep later optimization-series reports materially distinct from these four boundaries. The strongest remaining candidate is safe delegation of higher-level semantic operations to kernel, NIC, DPU, or other hardware-specific implementations without collapsing back into execution placement, architecture capability negotiation, or native-operation TCB accounting. Do not repackage verifier equivalence, stale-profile invalidation, cross-JIT fallback, execution provenance, or trust-surface measurement with another optimizer name.
...
6. Use all verified weekly Search Console and GA4 Drive export sets in every run. As rechecked on `2026-09-09`, the newest source set remains `2026-08-31..09-06`. Its Search Console date rows are present through `2026-09-05`, but under the configured three-day lag finalized analysis uses rows only through `2026-09-04`; `2026-09-06` is absent.
...
16. The optimization series reaches four reports with the September 9 publication, but do not create a thin public series hub yet. Revisit a dedicated hub only when report-level acquisition or navigation evidence shows that it would improve retrieval beyond the existing Daily Report index.
