...
1. Preserve the rolling ten-report mix mechanically from the actually published archive. After the `2026-08-30` adjacent GPU instrumentation-safety report, the newest ten remain **8 eBPF-centered / 0 pure Agent / 2 adjacent systems** because it displaces the remaining August 20 adjacent GPU report. The `2026-08-31` utilization-versus-allocatability report is also honestly adjacent systems; if published, it rotates the August 21 eBPF report out and restores **7 / 0 / 3**. Never repair the ratio through classification or by publishing an extra report.
...
4. For the next normal GPU/runtime report, prefer a fourth distinct invariant. Strong candidates include **distributed GPU coordination with an explicit correctness property**, host-side scheduling noise only if it goes beyond the August 20 launch-latency/causality reports, or another device/runtime boundary with measurable ground truth. Do not repeat memory placement, instrumentation non-interference, or utilization/allocatability with a different tool example.
5. Use all verified weekly Search Console and GA4 Drive export sets in every run. The configured folder was directly reverified on `2026-08-31`; the newest set remains `2026-08-17..23` and no later weekly set was observed.
...
9. Use finalized date-by-page or date-by-query evidence before attributing current search movement to one page or query family. The latest weekly page/query aggregates contain mixed movement, including higher clicks for the English hello-world page alongside worse average position, and lower clicks for the Chinese GPU-architecture page. Do not turn those aggregate movements into a title, copy, navigation, or site-structure change without the missing date dimension.
