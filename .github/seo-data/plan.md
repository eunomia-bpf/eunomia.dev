...
1. Preserve the rolling ten-report mix mechanically from the actually published archive. Before and after the `2026-08-27` report, the newest ten remain **7 eBPF-centered / 0 pure Agent / 3 adjacent systems** because the incoming eBPF-centered backend-conformance report ages another eBPF-centered report out.
2. Keep **eBPF Networking and Security** as the active series. Reports on `2026-08-23..27` now cover multi-owner composition, zero-copy ownership, temporal state correctness, bounded authorization revocation, and cross-backend security semantic conformance. Prefer one final distinct question around enforcement coverage across multiple hook families or another security property not already covered by placement, upgrade, composition, ownership, state transition, revocation, or backend conformance.
3. Use all verified weekly Search Console and GA4 Drive export sets in every run. The configured folder was directly reverified on `2026-08-27`; the newest set remains `2026-08-17..23`, and an explicit search found no `2026-08-24..30` set.
4. Treat the `2026-08-17..23` GA4 aggregate as finalized: 984 organic landing-page sessions at about 49.29% session-weighted engagement versus 970 at about 44.95% for `2026-08-10..16`. Sessions are about 1.4% higher and engagement about 4.34 percentage points higher. The weekly aggregate cannot support within-week causal attribution.
5. Keep complete GSC 7-day and 28-day comparisons unavailable until source history actually supports them. The latest finalized seven-day comparison would require `2026-08-16..22` versus `2026-08-09..15`, but the date exports omit both `2026-08-16` and `2026-08-09`; verified history is also too short for the 28-day comparison.
6. Use the longest equal-duration finalized GSC comparison supported by contiguous source rows: `2026-08-17..22` has 477 clicks / 59,798 impressions / ~0.798% CTR / ~9.56 weighted position versus 393 / 66,510 / ~0.591% / ~9.91 for `2026-08-10..15`. Clicks are about 21.4% higher, impressions about 10.1% lower, CTR about 0.207 percentage points higher, and weighted position improves by about 0.35 positions.
7. Use finalized date-by-page or date-by-query evidence before attributing search movement to one page or query family. The newest page/query exports remain broad and do not justify a title, navigation, or site-structure change for today's research topic.
...
10. Use search behavior, GitHub activity, primary research, kernel changes, and production evidence to order questions inside approved eBPF and adjacent systems series.
11. Revisit a dedicated public hub for completed eBPF series only after report-level acquisition or navigation evidence shows that it would improve retrieval beyond the existing Daily Report index.
12. Migrate the consuming SEO contract before moving the pinned `seo-skills` submodule from `516e9e2dcf012506a677a749049d64c5914643e9` to newer upstream `main`; upstream movement alone is not evidence that a pointer-only bump is safe.
