...
1. Preserve the rolling ten-report mix mechanically from the actually published archive. After the `2026-09-02` adjacent GPU membership/generation report, the newest ten contain **6 eBPF-centered / 0 pure Agent / 4 adjacent systems**. The `2026-09-03` megakernel-observability report is genuinely eBPF-centered because its central mechanism is a device-side eBPF semantic monitor, not optional host tracing. Its publication rotates an older eBPF-centered report out, so the newest-ten mix remains **6 / 0 / 4**. Never repair the ratio through classification or by publishing an extra report.
...
3. Keep **GPU and Heterogeneous Runtime Systems** as the active normal series. Its post-activation sequence now covers: `2026-08-29` evidence for GPU memory placement under oversubscription; `2026-08-30` instrumentation non-interference and observation coverage; `2026-08-31` candidate-conditioned allocatability; `2026-09-02` membership/generation continuity; and `2026-09-03` semantic observability after megakernel fusion. The fifth report is eBPF-centered because compiler/runtime task hooks are consumed by a late-bound device-side eBPF program type with bounded aggregation and explicit coverage.
4. Treat compiler-native megakernel profiling as the strongest baseline for the September 3 report and future follow-up. Do not claim a programmable eBPF layer is useful merely because it is dynamic. It must beat simpler compiler-native task tracing or PC-plus-metadata on late-bound query flexibility, evidence budget, perturbation, or diagnosis quality.
5. The GPU/runtime series has five substantial post-activation reports after September 3. A sixth report is allowed only when it exposes a genuinely distinct boundary with measurable ground truth. Do not repeat memory placement, instrumentation non-interference, allocatability, membership/generation continuity, launch causality, host/device causality, or megakernel semantic observability with a different tool example. Host-side scheduling noise is viable only if the question materially exceeds the site's existing CPU-noise-in-GPU-inference work; another distributed GPU question must go beyond communicator/state-generation continuity.
6. Use all verified weekly Search Console and GA4 Drive export sets in every run. As of `2026-09-03`, no export newer than `2026-08-24..30` is present. Its Search Console date file contains rows through `2026-08-29` with `2026-08-30` absent. Every row actually present is now outside the configured three-day lag.
...
15. Revisit a dedicated public hub for completed eBPF series only after report-level acquisition or navigation evidence shows that it would improve retrieval beyond the existing Daily Report index.
