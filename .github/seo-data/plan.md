...
1. Preserve the rolling ten-report mix mechanically from the actually published archive. After the `2026-09-04` checkpoint report the newest ten contain **5 eBPF-centered / 0 pure Agent / 5 adjacent systems**. The `2026-09-05` runtime-profile-specialization report is genuinely eBPF-centered because BPF bytecode, the verifier/JIT boundary, link generation, and BPF-specific observable effects are the central mechanism. It rotates the `2026-08-25` eBPF-centered report out, so the newest-ten mix remains **5 / 0 / 5** after publication. Never repair the ratio through classification or by publishing an extra report.
2. **eBPF Optimization and Execution Specialization** is the active series. The September 5 report establishes its first boundary: kernel verifier acceptance proves a specialized candidate is safe to execute, but does not prove that it preserves the original program's observable behavior or that workload-derived assumptions remain valid. The report develops an optimization-equivalence certificate, guarded profile dependencies with bounded deoptimization, and a phase-shift/rare-path benchmark.
3. Keep later optimization-series reports materially distinct from that first boundary. Good remaining candidates include architecture-specialization contracts that expose machine assumptions, safe delegation of operations to hardware-specific implementations, evidence that separates portable optimization value from one backend/microbenchmark, and debugging/provenance for specialized code. Do not repackage verifier-safety-versus-equivalence or stale-profile invalidation with another optimizer name.
...
6. Use all verified weekly Search Console and GA4 Drive export sets in every run. As of `2026-09-05`, no export newer than `2026-08-24..30` is present. Its Search Console date file contains rows through `2026-08-29` with `2026-08-30` absent.
...
12. The independent crawler/search view now discovers the September 4 checkpoint-recovery report and September 3 megakernel report as of `2026-09-05`. Treat this as supplementary retrievability evidence. Exact-SHA Pages deployment and generated production artifacts remain the stronger publication acceptance evidence.
