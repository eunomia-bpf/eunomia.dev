...
1. Preserve the rolling ten-report mix mechanically from the actually published archive. Before the `2026-09-07` publication the newest ten contain **5 eBPF-centered / 0 pure Agent / 5 adjacent systems**. The native-fast-path trust-boundary report is genuinely eBPF-centered because verifier-approved BPF semantics, architecture-specific native emits, translation validation, effect constraints, and kernel TCB are the central mechanism. It rotates the `2026-08-27` eBPF-centered report out, so the newest-ten mix remains **5 / 0 / 5** after publication. Never repair the ratio through classification or by publishing an extra report.
2. **eBPF Optimization and Execution Specialization** is the active series. September 5 established verifier-safety versus optimizer-equivalence and profile-assumption lifetime. September 6 established architecture-specific native implementation eligibility, a portable semantic witness, and deterministic fallback across JIT backends. September 7 advances a third boundary: even an eligible native implementation transfers trust from verifier-approved BPF semantics to the exact architecture-specific bytes that execute.
3. The September 7 report develops three artifacts for that trust boundary: proof-carrying native-operation capsules checked at load time; an effect-typed/restricted native emitter that constrains register, memory, stack, control-flow, helper, and target-feature effects; and a TCB-performance frontier benchmark that injects emitter and proof-linkage faults and measures mutant escape rate rather than speedup alone.
4. Keep later optimization-series reports materially distinct from these first three boundaries. A strong next candidate is debugging/provenance for dynamically specialized code: which semantic operation, native implementation, target predicate, checker result, and code hash actually executed, and how an operator can reconstruct that decision after an incident. Safe delegation of higher-level operations to hardware-specific implementations remains another candidate only if it does not repeat the completed heterogeneous execution-placement thesis. Do not repackage verifier-safety-versus-equivalence, stale-profile invalidation, capability negotiation, cross-JIT fallback, or today's TCB/translation-validation thesis under another optimizer name.
5. Treat **eBPF Networking and Security** as complete at its normal six-report boundary after the `2026-08-28` proxy handoff report. Return only when fresh evidence supports a mechanism beyond policy composition, zero-copy ownership, temporal state correctness, revocation, complete mediation, or proxy identity continuity.
6. Treat **GPU and Heterogeneous Runtime Systems** as complete at its normal six-report post-activation boundary after `2026-09-04`. Do not continue with a seventh report merely because another GPU paper exists. Its covered boundaries are memory-placement evidence, instrumentation non-interference, candidate-conditioned allocatability, membership/generation continuity, semantic observability after megakernel fusion, and application-consistent checkpoint/restore.
7. Use all verified weekly Search Console and GA4 Drive export sets in every run. As of `2026-09-07`, no source set newer than `2026-08-24..30` is present. Search Console date rows remain verified through `2026-08-29`, with `2026-08-30` absent.
8. Record the current Search Console `2026-08-24..29` slice as **436 clicks / 55,594 impressions / ~0.784% CTR / ~10.73 impression-weighted position**. The equal-duration `2026-08-17..22` slice is **477 / 59,798 / ~0.798% / ~9.56**. Current clicks are ~8.6% lower, impressions ~7.0% lower, CTR ~0.013 percentage points lower, and position ~1.17 worse. Label this as a six-day source-native comparison, not a complete seven-day trend.
9. Keep complete GSC 7-day and 28-day comparisons unavailable until source history supports them. The prior weekly export omits `2026-08-23`, the current set omits `2026-08-30`, and other older gaps prevent a complete preceding 28-day source window. Missing rows are never zero.
10. Weekly GSC page aggregates may prioritize inspection but must not be used for page-level causal claims without date-by-page evidence. Daily Report pages show 6 clicks / 1,017 impressions in the current weekly page export versus 5 / 744 previously, but volume is still too small to justify a title, navigation, or metadata change by itself.
11. Treat the GA4 `2026-08-24..30` organic landing-page aggregate as fully finalized. It contains 1,007 sessions at about 45.88% session-weighted engagement. The preceding finalized `2026-08-17..23` aggregate contains 984 sessions at about 49.29% engagement. Sessions are about **2.3% higher** week over week while engagement is about **3.41 percentage points lower**.
12. Use finalized date-by-page or date-by-query evidence before attributing search movement to one page, report, title, or topic family. Weekly page/query aggregates without a date dimension are prioritization evidence, not causal attribution.
13. Treat independent crawler/search discovery as supplementary retrievability evidence. As of the September 7 audit it exposes the September 5 runtime-profile report but has not yet refreshed the September 6 architecture-portability route. Exact-SHA Pages deployment and generated production artifacts remain the stronger publication acceptance evidence; crawler lag by itself is not a technical defect.
14. Treat GA4 `(not set)` and remaining legacy `/en/` traffic as measurement and technical SEO questions that require richer source-native evidence rather than as reasons to steer Daily Report topics.
15. Add Cloudflare coverage only when a supported read-only route is enabled in repository configuration.
16. Use search behavior, GitHub activity, primary research, kernel changes, and production evidence to order questions inside approved eBPF and adjacent systems series. Fresh kernel evidence such as the September 2026 JITed-BPF KASAN series can justify ordering when it exposes a durable mechanism rather than a one-off patch detail.
17. Revisit a dedicated public hub for a series only after at least three strong reports and report-level acquisition or navigation evidence show that it would improve retrieval beyond the existing Daily Report index. The optimization series reaches three reports with September 7, but no current report-level acquisition/navigation evidence yet justifies a thin public hub.
18. Migrate the consuming SEO contract before moving the pinned `seo-skills` submodule to a newer upstream layout. Upstream `main` remains newer at `f42128a3f05c73cf10c786a2711c488bb3a14839`, but upstream movement alone is not evidence that a pointer-only bump is safe.
