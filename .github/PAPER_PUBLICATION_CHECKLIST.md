...
- Add the paper's full text in both forms under `docs/papers/`: the PDF (`<slug>.pdf`) and a plain-text extraction (`<slug>.txt`). Only published papers or explicitly public preprints may enter this directory; private work under review must never appear here.
