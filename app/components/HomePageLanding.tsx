...
function ProjectCard({ project, locale }: { project: ProjectCardConfig; locale: Locale }) {
  ...
  return (
    <article className="group h-full rounded-lg border border-slate-200 bg-white p-4 transition hover:border-cyan-700/40 hover:bg-slate-50">
...
function groupProjects(
  catalog: ProjectCatalogConfig,
  enabledGroupKeys: string[]
): ProjectGroupWithProjects[] {
  const byKey = new Map(catalog.projects.map((project) => [project.key, project]));
  const enabled = new Set(enabledGroupKeys);

  return catalog.projectGroups
    .filter((group) => enabled.has(group.key))
    .map((group) => ({
      ...group,
...
function CapabilitySection({ home, locale }: { home: HomePageConfig; locale: Locale }) {
  return (
    <section className="border-b border-slate-200 pb-12" aria-labelledby="home-capabilities">
      <div className="mb-6 grid gap-4 lg:grid-cols-[18rem_minmax(0,1fr)]">
...
export function HomePageHero({ home, locale }: { home: HomePageConfig; locale: Locale }) {
  return (
    <section className="relative isolate overflow-hidden border-b border-slate-200 bg-[#07111f] text-white">
      ...
      <div className="relative mx-auto max-w-[82rem] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-end">
          ...
          <div className="grid gap-3 rounded-lg border border-white/20 bg-white/10 p-4">
            {home.hero.proofPoints.map((point) => (
              <div key={`${point.value}:${localizedText(point.label, locale)}`} className="border-b border-white/10 pb-3 last:border-b-0 last:pb-0">
                <p className="text-lg font-semibold text-white">{point.value}</p>
                <p className="mt-1 text-sm leading-6 text-slate-300">{localizedText(point.label, locale)}</p>
              </div>
            ))}
          </div>
        </div>
...
export function HomePageLanding({ locale, recentPosts, home, projectCatalog }: HomePageLandingProps) {
  const projectGroups = groupProjects(projectCatalog, home.projectGroupKeys);
  ...
  return (
    <div className="pb-16">
      ...
      <section className="border-b border-slate-200 py-12" aria-labelledby="home-projects">
        <div className="mb-7 grid gap-4 lg:grid-cols-[18rem_minmax(0,1fr)]">
