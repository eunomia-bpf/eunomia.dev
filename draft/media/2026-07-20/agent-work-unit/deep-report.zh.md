...
source_cutoff: 2026-07-20 00:23 PDT
status: draft
...
过去几年，AI 基础设施最容易回答的问题是一次模型调用发生了什么：用了哪个模型，输入和输出多少 token，延迟多长，是否报错。Agent 把这个问题拉长了。一次用户请求现在可能跨越几十次模型调用、多个工具、长时间暂停、人工审批、失败重试和外部系统状态变化。单次调用仍然可观测，却越来越难说明工作是否真的完成。

最近一轮产品与工程信号把这种错位推到了台前。7 月 17 日，OpenAI 提议用完成的有用工作、成功任务成本和结果可靠性衡量 AI；Google Cloud 把 Agent Platform 展示为从构建、持久运行、治理到评估的完整生命周期；GitHub 将 Copilot 指标下沉到仓库和 Pull Request 活动；阿里云则在 7 月 20 日开始对 AgentLoop 商业计费，并把 coding agent 的会话、工具调用、token、日志和 trace 纳入同一个观测系统。

这些发布来自不同产品线，却指向同一个变化：**Agent infra 正在从“模型调用管理”走向“工作管理”。问题在于，行业已经能记录一次工作的大部分过程，却还没有共同定义什么算完成、完成得是否正确，以及一次可靠完成究竟花了多少钱。**

## 四个同时出现、却不能混用的计量单位

OpenTelemetry 已经能把一次 Agent 执行拆成 `invoke_agent`、模型调用和 `execute_tool` 等 span，并记录模型、token、延迟和工具结果。它解决的是遥测层问题：系统做过什么，哪里慢，哪里失败。对于排查重试循环、慢工具和异常 token 消耗，这一层不可替代。
...
自动评估的困难不是理论上的担忧。7 月初的一项 coding agent benchmark 审计在四类云机器上重放 740 个性能优化任务，发现多个 benchmark 的参考补丁在跨机器环境中并不稳定；共同提交的排名还会随评分规则变化，28 对比较里有 9 对次序不一致。这个结果针对性能优化 benchmark，不足以否定所有 Agent 评估，却清楚说明了一个边界：即使有可执行测试和数值分数，环境、oracle 和聚合方式仍可能改变结论。

社区层面的摩擦也提醒我们，漂亮的生命周期图不会消除基础设施故障。Google Developer Forums 在 7 月 20 日仍有用户报告 Gemini Enterprise 自定义 MCP 的 reload 操作持续返回 401，也有人询问 Agent Studio 的多 Agent 编排和工具集成方式。这些帖子只是个案，不能代表平台整体可靠性，但它们展示了任务结果为何必须绑定外部状态：trace 可能完整记录了调用，用户需要的 action 却没有真正生效。
...
- OpenTelemetry 的 GenAI semantic conventions 能否从模型与工具 span 继续扩展到可移植的任务结果和评估事件；
- GitHub 后续的 repository insights 是否会加入测试通过、返工、time-to-merge 或缺陷回滚等结果信号；
- AgentLoop 商业化后是否出现包含准确率、总成本和人工复核的独立生产案例；
- 自动 judge 如何被校准、审计和版本化，尤其是 evaluator 本身也会调用工具和执行多步推理时；
- 哪些工作单元可以跨 Agent 与平台复用，哪些必须由具体业务工作流定义。
...
- [Alibaba Cloud, Commercialization Notice for AgentLoop, effective 2026-07-20](https://www.alibabacloud.com/en/notice/commercialization_notice_for_agentloop_795?_p_lc=1)
...
- [Chen et al., Are Performance-Optimization Benchmarks Reliably Measuring Coding Agents?, 2026-07-01](https://arxiv.org/abs/2607.01211)
- [Google Developer Forums, latest Agent and infrastructure topics, checked 2026-07-20](https://discuss.google.dev/c/google-cloud/14)
