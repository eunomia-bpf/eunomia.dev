<!--
Platform: Zhihu
Mode: canonical syndication
Title: eBPF 入门实践教程第五十篇：使用 TCX Link 实现可组合的流量控制
Source path: docs/tutorials/50-tcx/README.zh.md
Source URL for ledger: https://eunomia.dev/zh/tutorials/50-tcx/
Project source: https://github.com/eunomia-bpf/bpf-developer-tutorial/tree/main/src/50-tcx
Published URL: https://zhuanlan.zhihu.com/p/2067144957194924576
Question: linux内核如何实践？ (https://www.zhihu.com/question/60124044)
Topics: ebpf, Linux 内核
Media: source contains no inline images; use no cover unless Zhihu requires one
Formatting fixes: source H1 is carried by the Zhihu title field; the relative lesson-20 link is converted to its public eunomia.dev URL
Body policy: title and substantive body preserved exactly
QA state: confirmed on the public page
Public-page QA: exact title, 21 body headings, 8 code blocks, 2 tables with 12 rows, 10 outbound article links, no inline images, and no failure placeholders
-->
...
[第 20 课](https://eunomia.dev/zh/tutorials/20-tc/)讲的是经典 TC 路径：创建 `clsact` qdisc，挂载 `SEC("tc")` 程序作为 filter，用 `__sk_buff` 检查数据包。那一课仍然有价值，因为**数据面模型**是相同的，TCX 程序使用相同的上下文结构和相同的 helper 来访问数据包。
