...
- BPFix 日期修复：页脚的首次发布日期现在优先使用文章 front matter；TypeScript 和定向 ESLint 通过。Windows 构建在最后并行导出阶段遇到本机原生进程异常，但 GitHub Actions 的完整静态验证、生产构建、URL 检查和 Pages 部署均通过；无缓存请求与侧边栏浏览器确认中英文线上页分别显示 \`2026年7月25日\` 和 \`Jul 25, 2026\`，不再显示 7 月 21 日。
- 问题：<https://github.com/eunomia-bpf/eunomia.dev/issues/129> 中知乎已修复、掘金待审核；<https://github.com/eunomia-bpf/eunomia.dev/issues/130> 已在部署确认后关闭。
