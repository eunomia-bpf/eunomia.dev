<!--
Platform: Juejin
Mode: existing-article update
Title: eBPF 教程：检查 exec 后实际安装的可执行镜像
...
Media: the local exec-image-flow.png was uploaded after the remote image transfer failed
Body policy: source H1 is carried by the Juejin title field; all remaining source content is unchanged
QA state: update submitted on 2026-07-22; the staged /spost page shows the exact source title, 15 section headings, 8 code blocks, one table, one loaded 960x418 image, and the current system-wide lifecycle text; the public /post page retains the previous version while platform review is pending
-->
