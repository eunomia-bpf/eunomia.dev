...
Eunomia 不应该被表达成单纯卖一个软件产品的公司。更准确的公开定位是：Eunomia 是一个由 maintainer 牵引的 systems infrastructure lab / open-source infrastructure studio，围绕 AI agent、runtime、eBPF 和 GPU/system 边界做可验证的开源基础设施、研究和工程交付。
...
因此，网站和个人平台的语气应该像维护者和研究者在邀请高质量技术对话，而不是像传统 SaaS vendor 在推动注册试用。商业意图可以存在，但必须站在证据、开源、论文和真实问题之后；先让读者相信我们能判断复杂系统问题，再让合适的人进入具体合作。
...
- 三支柱区块比单纯项目卡片更重要。
- AgentSight、ActPlane、bpftime 的关系要清楚：agent 产品线是主叙事，bpftime 是 runtime engine 和技术护城河，tutorial/docs 是信任底盘。
...
2. Products 下拉顺序调整为 Agent 产品线、Services、bpftime。
