...
> 状态：内部执行 checklist，不发布。更新时间：2026-09-22。
...
- 剩余队列：知乎 24 条、掘金 34 条、Medium 0 条、DEV 0 条、LinkedIn 0 条，共 58 个平台任务；其中掘金 43-kfuncs 已提交并等待审核（`review_pending`），不计为已完成。后续新增内容直接插入合适位置。
...
- [ ] `阻塞` 掘金：`docs/tutorials/43-kfuncs/README.zh.md`。2026-09-22 使用 09-22 正常额度已通过可见编辑器导入并提交，提交页返回“发布成功”，暂存页 <https://juejin.cn/spost/7688489073967251494>；审核中，`/post/` 尚返回“找不到页面”。暂存页已通过结构 QA：原标题、正文单份 11047 字符、9 个 H2、11 个 H3、3 个 H4、20 个代码块（2 c、2 makefile、10 bash、2 sh、4 txt 输出块保持源文裸围栏，含有序列表内 7 个缩进围栏，与源文一一对应无重复）、9 条外链、1 张外链图片（raw.githubusercontent 直连 800×500，无转存失败标记），分类后端，标签 `Linux`、`后端`、`性能优化`。恢复条件：审核通过后 `/spost/` 跳转公开 `/post/7688489073967251494`，届时完成公开页 QA 并把本行改为 `[x]` 且 ledger 改为 `confirmed`；未确认前不得重复提交同一源文。
