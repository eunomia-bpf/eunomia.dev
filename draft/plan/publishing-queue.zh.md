...
- 剩余队列：知乎 24 条、掘金 36 条、Medium 0 条、DEV 0 条、LinkedIn 0 条，共 60 个平台任务；其中掘金 45-scx-nest 已提交并等待审核（`review_pending`），不计为已完成。后续新增内容直接插入合适位置。
...
- [ ] `阻塞` 掘金：`docs/tutorials/45-scx-nest/README.zh.md`。2026-09-15 使用 09-15 正常额度已通过可见编辑器导入并提交，提交页返回“发布成功”，暂存页 <https://juejin.cn/spost/7685561582916009994>；审核中，`/post/` 尚返回“找不到页面”。暂存页已通过结构 QA：原标题、正文单份 24628 字符、6 个 H2、6 个 H3、21 个 H4、24 个代码块（21 c、2 bash、1 个输出块保持源文裸围栏）、8 条外链、无图片，分类后端，标签 `Linux`、`后端`、`性能优化`。恢复条件：审核通过后 `/spost/` 跳转公开 `/post/7685561582916009994`，届时完成公开页 QA 并把本行改为 `[x]` 且 ledger 改为 `confirmed`；未确认前不得重复提交同一源文。
