...
Model note: the actual writing/editing pass runs on `claude-opus-4-6[1m]` (Opus 4.6, 1M context). When the calling agent is a different model, run the writing job through headless `claude -p "<prompt>" --model 'claude-opus-4-6[1m]'` in the background; do not use agent definitions for prose work.
...
4. **Terminology map first (mandatory before any ZH prose).** Build the post's EN→ZH term map from the rulebook's Chinese terminology discipline: which terms stay English (the four allowed classes), which translate (one rendering each), and which get a first-use gloss like 策略（policy）. Keep the map in your working notes and follow it for the entire ZH draft.
5. Write the Chinese version with identical section structure and figure/table placement, written as Chinese-first prose (not sentence-by-sentence translation); reorder within sentences freely so every paragraph passes the rulebook's read-aloud test.
6. Run Flow B (review), including its terminology check, on your own draft before reporting back.
...
4. **Terminology check (dedicated pass, ZH files).** Apply the rulebook's Chinese terminology discipline paragraph by paragraph: every English token must fall into one of the four allowed classes; concept nouns with standard renderings must be translated with one consistent form; no English-headed Chinese sentences (`grep -nE '^[A-Z][A-Za-z-]+ ' file.zh.md` on prose lines must be empty); table headers Chinese; quotations rendered in Chinese; each paragraph passes the read-aloud test. Also check CJK-Latin spacing, full-width punctuation, and no English clause splicing. Violations are Must fix.
