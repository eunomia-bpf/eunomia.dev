...
## Paper figure completeness

Before drafting from a paper, inventory every main-body figure and table by number, caption, source asset, and the claim it supports. A full paper-based post includes every main-body figure whose section remains in the article. When the topic is the empirical study, every figure from the paper's empirical-study section is mandatory. If the article also retains design or evaluation content, include the corresponding design and evaluation figures as well. Because a style-only rewrite preserves content, it cannot omit a source figure for a retained result.

Prefer repository-owned copies of source figures under the post's image directory. Keep image payloads identical across EN/ZH, localize alt text and surrounding explanation, and place each figure immediately after the paragraph that introduces its claim. Any omitted paper figure needs an explicit scope reason in the completion report; “the draft was already long” is not sufficient.
...
2. Write an **architecture brief** before prose: one-sentence thesis, reader promise, unique angle relative to sibling posts, non-goals, opening scenario or measurement, and a 3-6 H2 progression. For paper-based posts, state why this outline does not mirror the paper's sections or RQs.
...
4. Check frontmatter and SEO metadata from the SEO rulebook, including title length, description length, primary keyword placement, sibling cannibalization, and internal links.
...
- Paper figure inventory recorded: every retained paper section has its corresponding figures, all empirical-study figures are present, EN/ZH placement matches, and every omission has a scope justification.
