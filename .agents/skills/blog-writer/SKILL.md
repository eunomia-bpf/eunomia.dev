...
2. Write an **architecture brief** before prose: one-sentence thesis, reader promise, unique angle relative to sibling posts, non-goals, opening scenario or measurement, and a 3-6 H2 progression chosen for this argument rather than copied from a house template. Name the structural form being used and why it fits the material, then confirm that nearby posts do not use an unnecessarily identical skeleton. Generate several truthful title candidates that emphasize different source-backed stakes, then choose the most compelling candidate that remains precise, differentiated, keyword-aware, professional, and faithful. Reject any candidate whose appeal depends on withheld information, inflated certainty, emotional manipulation, or content-farm phrasing. For paper-based posts, state why this outline does not mirror the paper's sections or RQs.
