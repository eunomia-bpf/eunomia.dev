...
1. **Codex drafts.** Read the primary source and relevant sibling posts. Write a one-sentence thesis and identify the target reader, distinctive insight, evidence, mechanism, boundary, and practical decision. Before outlining, write a background contract naming what the target reader likely knows, what the post must explain before its first specialized claim, and which sibling link can carry deeper prerequisites. Every blog needs a short ramp from reader context to the new claim, so do not open by jumping straight into a tool, paper result, benchmark, release note, or implementation detail. The frontmatter `description` and the excerpt before `<!-- more -->` must also begin with one compact background clause or sentence that names the problem domain or reader situation before stating the post's contribution. For paper-based posts, build a main-body figure and table inventory before outlining, with each item's supported claim, source asset, and include/omit decision. Draft or revise the complete EN/ZH pair, then read each paragraph once for information flow, sentence rhythm, and density. The reader should not have to wait for a missing object, reconstruct a broken causal chain, or unpack several new numbers and terms at once. For new work, use `draft/blog/` until the public filename, date, and slug are settled, and keep workflow notes, tutorial-vs-blog planning, review logs, and figure inventories in separate draft notes rather than public post prose.
...
- Treat each post as one personal-brand content unit, not only as a website
  page. The post should strengthen the maintainer's public association with
  eBPF systems, AI-agent infrastructure, research taste, and open-source
  teaching, with eunomia-bpf projects used as evidence rather than as the only
  subject.
- Prepare for a two-day publishing cadence by making the core claim reusable:
  one clear platform-native hook, one evidence sentence with a source, and one
  contextual project or canonical archive link that can be adapted for X,
  LinkedIn, Zhihu, Juejin, Reddit, or HN without sounding like a link dump.
...
- the frontmatter `description`, excerpt, opening, and first section give enough domain background for the target reader before the first specialized result, tool claim, benchmark, or implementation detail;
